...
def knn(dataTest,k,fdest,df_train):
    ...
    #calcul le label le plus présent dans le dictionnaire de résultat
...
def main():
    ...
    if len(sys.argv) == 3:
        df_train=pd.read_csv(sys.argv[1], names=["a","b","c","d","e","f","g","label"])
        df_try=pd.read_csv(sys.argv[2], names=["a","b","c","d","e","f","g","label"])
        for index, row in df_try.iterrows():
            dataTest = np.array((row['a'], row['b'],row['c'], row['d'], row['e'], row['f'], row['g']))
            knn(dataTest,k,fdest,df_train)
        fdest.close()
